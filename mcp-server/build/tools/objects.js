...
`;
export function registerObjectTools(server, client) {
    server.tool("speckle_get_object", "Get a Speckle object by ID from a project. Returns the object data, speckle type, and children count.", {
    ...
    server.tool("speckle_get_object_children", "Get child objects of a Speckle object with pagination and optional property selection. Use 'select' to request only specific properties.", {
        projectId: z.string().describe("The project ID"),
        objectId: z
            .string()
        ...
        select: z
            .array(z.string())
            .optional()
            .describe("Property names to include in response (e.g. ['speckle_type', 'name', 'area'])"),
        depth: z
            .number()
            .min(1)
            .max(10)
